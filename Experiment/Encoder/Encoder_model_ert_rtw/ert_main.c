...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Encoder_model_M->Timing.taskTime0;
  Encoder_model_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM4_Init();
  MX_TIM8_Init();
  rtmSetErrorStatus(Encoder_model_M, 0);
  ...
  Encoder_model_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Encoder_model_M->extModeInfo, &rtmGetTFinal
    (Encoder_model_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Encoder_model_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Encoder_model_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
